---
title: "MatrisomeDB: RNA ~ protein correlation in CCLE dataset"
...
## Motivation
> Fibrosis stems from the accumulation of excess deposition of extracellular 
matrix components (ECM), which results in thickening tissue. We measure ECM-related
gene expression as a proxy of cells driving this process, but the actual encoding 
protein levels might not be associated to this. Herein we compare mRNA ~ protein
levels on this regard.
...
First we load the suppl tab 4 from the original paper, with HT-wide correlation 
...
Load the gene set collection defined as part of MatrisomeDB
```{r gsc}
...
```{r vis, fig.width=5, fig.height=3, dpi=300}
give.n <- function(x){
